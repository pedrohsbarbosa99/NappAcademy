...
class Pedido:
    formas_aceitas = ['dinheiro', 'cartao', 'pix']
    def __init__(self, cliente):
        if isinstance(cliente, Cliente):
            self._cliente = cliente
            self._itens = []
            return
        raise TypeError('Nao e possivel instanciar um Pedido sem um cliente')
    ...
    def add_item(self, produto):
        if isinstance(produto, Produto):
            ...
            return
        raise TypeError('Nao foi passado um objeto produto')
    ...
    def checkout(self, forma_pagamento=None):
        ...
        raise Exception('Forma de pagamento nao aceita')
